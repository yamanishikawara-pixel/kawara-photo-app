    ...
    try {
      ...
      if (!serverOk) {
        ...
        try {
          ...
          setError(
            'PDFの保存に失敗しました。ページを再読み込みしてから再度お試しください。',
          );
        }
      }
    } catch (err: unknown) {
    ...
    <div className="min-h-screen bg-gray-200 p-4 sm:p-6 font-sans flex flex-col items-center pb-12 overflow-x-hidden w-full relative">
      ...
      <div className="flex flex-col gap-8 items-center w-full">
        ...
        <div style={wrapperStyle} className="relative bg-white shadow-md shrink-0">
          {/* ★font-sans を追加して常にゴシック体に！ */}
          <div
            className="pdf-page absolute top-0 left-0 flex flex-col items-center origin-top-left font-sans"
            style={{ ...pageStyle, backgroundColor: '#ffffff', color: '#000000' }}
          >
            {/* ★ロゴ巨大化バグ修正： style={{ width: '40mm' }} 等でサイズを強制固定 */}
            <div className="flex flex-col items-center w-full" style={{ marginTop: '5mm', marginBottom: '28mm' }}>
              <div className="shrink-0 flex justify-center mb-6">
                {logoUrl ? (
                  <img src={proxyUrl(logoUrl, `logo_${sessionId}`)} alt="自社ロゴ" className="block h-auto object-contain" style={{ width: '40mm' }} crossOrigin="anonymous" />
                ) : (
                  <img src={kawaraLogo} alt="標準ロゴ" className="block h-auto object-contain grayscale" style={{ width: '32mm' }} crossOrigin="anonymous" />
                )}
              </div>
            ...
            <div className="flex flex-col" style={{ width: '150mm', gap: '12mm' }}>
              {COVER_FIELDS.map((item, idx) => {
                ...
                return (
                  <div key={idx} className="flex items-end pb-2" style={{ borderBottom: '2px solid #000000' }}>
                    <div className="flex-shrink-0 flex justify-between text-[22px] font-bold pr-8 leading-none" style={{ width: '45mm' }}>
            ...
            {userSettings && (address || phone) && (
              <div className="absolute text-right flex flex-col items-end pl-4 py-1" style={{ bottom: '16mm', right: '15mm', backgroundColor: '#ffffff' }}>
            ...
            <div className="absolute text-[16px] font-bold" style={{ bottom: '10mm', right: '15mm', color: '#000000' }}>
              - 1 / {totalPages} -
            </div>
          </div>
        </div>
        ...
        {mapUrlsToRender.map((u, mapIndex) => (
          <div key={`map-page-${mapIndex}`} style={wrapperStyle} className="relative bg-white shadow-md shrink-0">
            <div
              className="pdf-page absolute top-0 left-0 flex flex-col origin-top-left font-sans"
              style={{ ...pageStyle, backgroundColor: '#ffffff', color: '#000000' }}
            >
              ...
              <div className="absolute text-xs font-serif" style={{ bottom: '10mm', right: '15mm', color: '#9ca3af' }}>
                      ...
                      {p.image ? (
                        <div className="flex items-center justify-center w-full h-full">
                          <div className="relative inline-block" style={{ transform: `rotate(${(p as Photo).rotation ?? 0}deg)`, transformOrigin: 'center center' }}>
                            {/* ★写真枠のはみ出し防止 */}
                            <img src={proxyUrl(p.image, `photo_${p.id}_${sessionId}`)} crossOrigin="anonymous" className="block w-auto h-auto max-w-full" style={{ maxHeight: '88mm' }} alt="" />
                      ...
                      {m.image ? (
                        <div className="flex items-center justify-center w-full h-full">
                          <div className="relative inline-block" style={{ transform: `rotate(${m.rotation ?? 0}deg)`, transformOrigin: 'center center' }}>
                            <img src={proxyUrl(m.image, `material_${m.id}_${sessionId}`)} crossOrigin="anonymous" className="block w-auto h-auto max-w-full" style={{ maxHeight: '85mm' }} alt="" />
                          </div>
                        </div>
                      ) : (
                        <span className="font-bold" style={{ color: '#9ca3af' }}>写真未登録</span>
