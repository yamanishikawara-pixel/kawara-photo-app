...
function PhotoCircleMarker({
  ...
  return (
    <>
      ...
      />
      {isSelected && !dragging && (
        <div style={{ left: `${position.x}%`, top: `${position.y + circle.size/2 + 5}%`, transform: 'translateX(-50%)' }} className="absolute z-[60] flex bg-white rounded-xl shadow-xl border border-gray-200 overflow-hidden">
    ...
    <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-6 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white rounded-3xl w-full max-w-sm p-6 shadow-xl space-y-5" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center pb-2 border-b">
          <h3 className="text-xl font-bold text-gray-900 flex items-center gap-2"><MapPin className="text-red-500 w-6 h-6"/> 位置の選択</h3>
          <button onClick={onClose} className="p-1 text-gray-400 hover:text-gray-600"><X className="w-6 h-6"/></button>
        </div>
        {pins && pins.length > 0 ? (
          <div className="grid grid-cols-3 gap-3">
            {pins.map((pin) => (
              <button key={pin.id} onClick={() => { onSelect(pin.label); onClose(); }} className="bg-gray-100 text-gray-800 border-2 border-gray-200 font-bold py-3 text-center rounded-xl text-lg shadow-sm hover:border-red-400 hover:bg-red-50 transition-all">{pin.label}</button>
            ))}
          </div>
        ) : (
...
export default function PhotoPage() {
  ...
  const updatePhoto = async (photoId: number, field: keyof Photo, value: any) => {
    if (!project) return;
  ...
  const addCircleToPhoto = async (e: MouseEvent<HTMLDivElement>, photoId: number) => {
    ...
    const newPhotos = project.photos.map((p) => p.id === photoId ? { ...p, circles: [...(p.circles || []), { id: newId, x, y, size: 20 }] } : p);
    setProject({ ...project, photos: newPhotos });
    setSelectedCircleId(newId);
    await updateDoc(doc(db, "projects", id!), { photos: newPhotos });
  };

  if (!project) return <div className="p-10 text-center font-bold">読み込み中...</div>;

  return (
    <div className="min-h-screen bg-gray-50 p-4 font-sans" onClick={() => setSelectedCircleId(null)} lang="ja">
      <div className="max-w-md mx-auto pb-20">
        <button onClick={() => navigate(`/project/${id}`)} className="flex items-center gap-2 text-blue-500 mb-6 font-bold"><ArrowLeft /> もどる</button>
          ...
          {project.photos.map((photo, index) => (
            <div key={photo.id} className="bg-white p-5 rounded-3xl border border-black/5 shadow-md relative">
              <div className="absolute top-4 right-4 flex gap-2 z-10">
                <button onClick={() => movePhoto(index, 'up')} className="bg-white p-2 rounded-lg border shadow-sm"><ArrowUp className="w-5 h-5 text-gray-600" /></button>
                <button onClick={() => movePhoto(index, 'down')} className="bg-white p-2 rounded-lg border shadow-sm"><ArrowDown className="w-5 h-5 text-gray-600" /></button>
              </div>
              ...
              <div className="w-full aspect-video bg-gray-100 rounded-2xl overflow-hidden border-2 border-dashed border-gray-300 relative mb-4 mt-8 flex items-center justify-center">
                {loadingId === photo.id ? (
                  ...
                  <div className="relative w-full h-full" onClick={(e) => addCircleToPhoto(e, photo.id)}>
                    <img src={proxyUrl(photo.image, photo.id)} crossOrigin="anonymous" className="block w-full h-full object-contain pointer-events-none" style={{ transform: `rotate(${photo.rotation || 0}deg)` }} alt="" />
                    {(photo.circles || []).map((circle) => (
                      <PhotoCircleMarker key={circle.id} circle={circle} isSelected={selectedCircleId === circle.id} onSelect={() => setSelectedCircleId(circle.id)} onDragEnd={(x, y) => {
                        const newPhotos = project.photos.map(p => p.id === photo.id ? { ...p, circles: p.circles.map(c => c.id === circle.id ? { ...c, x, y } : c) } : p);
                        setProject({ ...project, photos: newPhotos });
                        void updateDoc(doc(db, "projects", id!), { photos: newPhotos });
                      }} onSizeChange={(size) => {
                        const newPhotos = project.photos.map(p => p.id === photo.id ? { ...p, circles: p.circles.map(c => c.id === circle.id ? { ...c, size } : c) } : p);
                        setProject({ ...project, photos: newPhotos });
                        void updateDoc(doc(db, "projects", id!), { photos: newPhotos });
                      }} onRemove={() => {
                        const newPhotos = project.photos.map(p => p.id === photo.id ? { ...p, circles: p.circles.filter(c => c.id !== circle.id) } : p);
                        setProject({ ...project, photos: newPhotos });
              ...
              <div className="flex justify-between items-center mb-5 border-b pb-4">
                <div className="font-bold text-xl text-gray-800">写真 {index + 1}</div>
                <div className="flex gap-2">
                  <button onClick={() => updatePhoto(photo.id, 'rotation', ((photo.rotation || 0) + 90) % 360)} className="p-2.5 text-gray-500 bg-gray-50 rounded-xl border">↻ 回転</button>
                  <button onClick={() => deletePhotoSlot(photo.id)} className="p-2.5 text-gray-400 hover:text-red-500 bg-gray-50 rounded-xl border"><Trash2 className="w-5 h-5"/></button>
              ...
              <div className="space-y-5">
                <div className="flex items-center gap-3">
                  <span className="font-bold text-gray-600 min-w-[5rem] text-lg">撮 影 日:</span>
                  <input type="date" className="w-full p-3 border rounded-xl text-lg focus:ring-2 focus:ring-blue-500" value={formatToYMD(photo.shootingDate)} onChange={(e) => updatePhoto(photo.id, "shootingDate", formatToYMDSlash(e.target.value))} />
                </div>
                <div className="flex flex-col gap-1">
                  <span className="font-bold text-gray-600 text-lg ml-1">工　程:</span>
                  <select className="w-full p-4 border rounded-xl bg-white text-lg font-bold" value={photo.process} onChange={(e) => updatePhoto(photo.id, "process", e.target.value)}>
                    <option value="">-- 工程を選択してください --</option>
                    {processOptions.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                  </select>
                </div>
                <button onClick={() => { setCurrentPhotoId(photo.id); setModalOpen(true); }} className={`w-full p-4 border-2 rounded-xl text-left flex justify-between items-center text-lg ${photo.locationMap ? 'text-red-700 font-bold border-red-300 bg-red-50' : 'text-gray-500'}`}>
                  {photo.locationMap || '▼ 位置図を選択'} <MapPin />
                </button>
                <div className="flex flex-wrap gap-2">
                  {descTemplates.map((tmpl, i) => (
                    <button key={i} onClick={() => updatePhoto(photo.id, "description", (photo.description || '') + tmpl.text)} className="text-sm font-bold text-blue-700 bg-blue-50 border border-blue-200 px-3 py-2 rounded-lg hover:bg-blue-100">＋{tmpl.label}</button>
                  ))}
                </div>
                <textarea rows={3} className="w-full p-4 border rounded-xl text-lg focus:ring-2 focus:ring-blue-500" value={photo.description} onChange={(e) => updatePhoto(photo.id, "description", e.target.value)} placeholder="説明入力" />
              </div>
            </div>
          ))}
        </div>
        
        <button onClick={addPhotoSlot} className="w-full mt-10 bg-gray-800 text-white font-bold py-5 rounded-2xl shadow-xl flex items-center justify-center gap-2 hover:bg-gray-700 text-xl">
          <Plus className="w-7 h-7" /> 写真枠を追加
        </button>
      </div>
