      ...
      <div className="bg-white rounded-[2rem] w-full max-w-sm p-8 shadow-2xl space-y-6" onClick={e => e.stopPropagation()}>
        ...
        {pins && pins.length > 0 ? (
          <div className="grid grid-cols-3 gap-4 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
            ...
            <button onClick={() => { onSelect(""); onClose(); }} className="col-span-3 bg-gray-100 text-gray-500 font-bold py-4 rounded-2xl mt-2">選択を解除</button>
          </div>
        ) : (
          <div className="text-center py-12 px-4 bg-gray-50 rounded-3xl border-4 border-dashed border-gray-200"><p className="text-gray-400 font-bold">先に位置図でピンを打ってください</p></div>
        )}
      </div>
    </div>
  );
      ...
      <div className="max-w-2xl mx-auto pb-12">
        ...
        <div className="bg-white p-6 rounded-[2.5rem] border-2 border-gray-100 shadow-sm mb-12">
          <label className="flex items-center justify-center gap-4 w-full bg-blue-600 text-white font-black py-6 text-2xl rounded-3xl cursor-pointer shadow-[0_15px_40px_rgba(37,99,235,0.4)] hover:bg-blue-700 transition-all active:scale-95">
            <UploadCloud className="w-8 h-8" /> {bulkUploading ? `アップロード中... (${bulkProgress}枚)` : "複数写真を一括追加する"}
        ...
        <div className="space-y-16 mt-4">
          {project.photos.map((photo, index) => (
            <div key={photo.id} className="bg-white p-8 rounded-[3rem] border-2 border-gray-100 shadow-2xl relative animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="w-full min-h-[22rem] mt-12 bg-[#f1f5f9] rounded-[2.5rem] flex items-center justify-center overflow-hidden border-4 border-dashed border-gray-200 relative mb-10 group">
                {loadingId === photo.id ? (
                  ...
                  <div className="relative inline-block" onClick={(e) => addCircleToPhoto(e, photo.id)}>
                    ...
                    <div className="absolute -top-6 -left-6 bg-black/80 backdrop-blur text-white text-sm px-6 py-3 rounded-full font-black pointer-events-none shadow-2xl border-2 border-white/20">タップで赤丸を追加</div>
                  </div>
                ) : (
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-4">
                  ...
                  <textarea rows={4} className="w-full p-8 text-2xl font-bold border-4 border-gray-100 rounded-[2.5rem] bg-gray-50 focus:border-blue-500 outline-none shadow-inner" value={photo.description} onChange={(e) => updatePhoto(photo.id, "description", e.target.value)} placeholder="現場状況の詳細を入力" />
                </div>
              </div>
            </div>
          ))}
