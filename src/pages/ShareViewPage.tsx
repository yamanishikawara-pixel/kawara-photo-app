        ...
        {mapUrls.length > 0 && mapUrls.map((u, mapIndex) => {
          ...
          return (
            <div key={`map-${mapIndex}`} className="rounded-2xl border overflow-hidden" style={{ background: CARD, borderColor: BORDER }}>
              ...
              <div className="overflow-hidden flex items-center justify-center" style={{ background: CARD2, maxHeight: '70vh' }}>
                    ...
                    {(project.mapDimensionLines ?? []).filter(l => (l.mapIndex || 0) === mapIndex).map((line) => {
                      ...
                      return (
                        <div key={line.id} className="absolute inset-0 z-20 pointer-events-none w-full h-full" style={{ overflow: 'visible' }}>
                          ...
                          {line.text && (
                            <div style={{ left: `${midX}%`, top: `${midY}%`, color, backgroundColor: 'rgba(0,0,0,0.6)', fontSize: '12px', transform: `translate(-50%, -50%) rotate(${line.textRotation ?? 0}deg)` }} className="absolute font-bold px-1 py-0.5 rounded whitespace-nowrap">
                              {line.text}
                            </div>
                          )}
                        </div>
              ...
              {/* 凡例テーブル */}
              {project.showLegendTable !== false && rows.length > 0 && (
                    ...
                    <tbody>
                      {rows.map(row => (
                        <tr key={row.id} style={{ borderBottom: `1px solid ${BORDER}` }}>
                          <td className="px-2 py-1.5 text-center font-bold border" style={{ borderColor: BORDER, color: '#ef4444' }}>{row.symbol}</td>
