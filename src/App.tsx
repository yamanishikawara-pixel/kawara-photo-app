...
function PhotoScreen() {
  ...
  const [modalOpen, setModalOpen] = useState(false);
  const [currentPhotoId, setCurrentPhotoId] = useState<number | null>(null);
  const [selectedCircleId, setSelectedCircleId] = useState<number | null>(null); // 選択中の赤丸
  ...
  const addCircleToPhoto = async (e: any, photoId: number) => {
    ...
    const newPhotos = project.photos.map((p: any) => {
      if (p.id === photoId) {
        return { ...p, circles: [...(p.circles || []), { id: Date.now(), x, y, size: 20 }] }; // sizeは幅の20%
      }
      return p;
    });
    setProject({ ...project, photos: newPhotos });
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 font-sans overflow-x-hidden" onClick={() => setSelectedCircleId(null)}>
      <div className="max-w-md mx-auto pb-12">
        ...
        <div className="space-y-8 mt-4">
          {project.photos.map((photo: any, index: number) => (
            <div key={photo.id} className="bg-white p-5 rounded-3xl border border-black/5 shadow-md relative">
              ...
              {/* ★ 変更：画像を大きくし、タップで赤丸が打てるようにしました！ */}
              <div 
                className="w-full h-56 bg-gray-100 rounded-2xl flex items-center justify-center overflow-hidden border-2 border-dashed border-gray-300 relative mb-4"
                  ...
                  <>
                    ...
                    {(photo.circles || []).map((circle: any) => (
                      <PhotoCircleMarker 
                        ...
                        onDragEnd={(x: number, y: number) => updateCircle(photo.id, circle.id, { x, y })}
                        onSizeChange={(size: number) => updateCircle(photo.id, circle.id, { size })}
              ...
              <div className="flex justify-between items-center mb-5 pb-4 border-b border-gray-100">
                ...
                <div className="flex gap-2">
                  {photo.image && (
                    <button onClick={() => updatePhoto(photo.id, "image", null)} className="p-2 text-gray-400 hover:text-red-500 bg-gray-50 rounded-lg"><Trash2 className="w-5 h-5"/></button>
                  )}
        ...
        <button onClick={addPhotoSlot} className="w-full mt-8 bg-gray-800 text-white font-bold py-4 text-lg rounded-xl shadow-lg flex items-center justify-center gap-2">
          <Plus className="w-6 h-6" /> 写真枠を1つ追加する
        </button>
...
function MapMarker({ pin, onDragEnd, onClick }: any) {
  ...
  return (
    <div
      ...
      style={{ left: `${position.x}%`, top: `${position.y}%`, transform: 'translate(-50%, -50%)', touchAction: 'none' }}
      className={`absolute flex items-center justify-center cursor-pointer transition-transform ${dragging ? 'z-30 scale-125 opacity-80' : 'z-10 hover:scale-110'}`}
    >
      {pin.type === 'arrow' ? (
        // 矢印（➡）デザイン
      ...
      <div className="bg-white rounded-3xl w-full max-w-sm p-6 space-y-6" onClick={e => e.stopPropagation()}>
        ...
        <div>
          ...
          <div className="flex gap-4">
            <button onClick={() => setType('circle')} className={`flex-1 py-3 font-bold border-2 rounded-xl flex items-center justify-center gap-2 ${type==='circle' ? 'border-red-500 bg-red-50 text-red-600' : 'border-gray-200 text-gray-500'}`}>
              <div className="w-5 h-5 rounded-full border-[3px] border-current"></div> 範囲 (〇)
            </button>
            <button onClick={() => setType('arrow')} className={`flex-1 py-3 font-bold border-2 rounded-xl flex items-center justify-center gap-2 ${type==='arrow' ? 'border-red-500 bg-red-50 text-red-600' : 'border-gray-200 text-gray-500'}`}>
          ...
          <div className="pt-2">
            ...
            <div className="grid grid-cols-4 gap-2">
              {[ {d: -90, l: '↑'}, {d: 0, l: '➡'}, {d: 90, l: '↓'}, {d: 180, l: '⬅'} ].map(rot => (
                <button key={rot.d} onClick={() => setRotation(rot.d)} className={`p-3 text-2xl font-black border-2 rounded-xl flex justify-center ${rotation === rot.d ? 'border-red-500 bg-red-100 text-red-600' : 'border-gray-200 text-gray-400'}`}>
...
function MapScreen() {
  ...
  const addPin = async (e: any, mapIndex: number) => {
    ...
    const label = String(currentPins.length + 1); // 自動で「1」「2」となる
                  ...
                  <div className="relative inline-block" style={{ maxWidth: '100%', maxHeight: '100%' }}>
                    ...
                    {(project.mapPins || []).filter((p: any) => p.mapIndex === mapIndex).map((pin: any) => (
                      <div key={pin.id} style={{ left: `${pin.x}%`, top: `${pin.y}%`, transform: 'translate(-50%, -50%)' }} className="absolute z-10">
                        {pin.type === 'arrow' ? (
                          <div className="flex items-center gap-1 bg-white/70 px-1 rounded">
                            <span className="text-red-600 font-black text-[22px]" style={{ transform: `rotate(${pin.rotation || 0}deg)` }}>➡</span>
                            <span className="text-red-600 font-bold text-[18px]">{pin.label}</span>
                          </div>
                        ) : (
                          <div className="relative flex items-center justify-center">
                            <div className="w-[12mm] h-[12mm] rounded-full border-[3px] border-red-600"></div>
                            <span className="absolute text-red-600 font-bold text-[16px] bg-white/80 px-1 rounded">{pin.label}</span>
                          </div>
                        )}
                      </div>
                    ))}
