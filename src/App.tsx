...
function PhotoScreen() {
  ...
  const handleBulkUpload = async (e: any) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (targetIndex === -1) {
        newPhotos.push({
          id: Date.now() + Math.random(), image: null, photoNumber: String(newPhotos.length + 1), shootingDate: "", locationMap: "", process: "", description: "", circles: []
        });
  ...
  const addCircleToPhoto = async (e: any, photoId: number) => {
    ...
    const newPhotos = project.photos.map((p: any) => {
      if (p.id === photoId) {
        return { ...p, circles: [...(p.circles || []), { id: Date.now(), x, y, size: 20 }] };
      }
      return p;
    });
    setProject({ ...project, photos: newPhotos });
