        ...
        <div className="space-y-3">
          {projects.map((p: any) => (
            <div key={p.id} className={`relative flex items-center p-4 rounded-2xl border transition-all ${p.id === currentProjectId ? 'bg-blue-50 border-blue-200 shadow-md ring-2 ring-blue-500/20' : 'bg-white border-black/5 shadow-sm'}`}>
              ...
              {projects.length > 1 && (
                <button onClick={() => window.confirm('この現場データを完全に削除しますか？') && deleteProject(p.id)} className="p-2 text-gray-300 hover:text-red-500 transition-colors"><Trash2 className="w-5 h-5" /></button>
              )}
            </div>
          ))}
        </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-orange-50 p-6 font-sans">
      <div className="max-w-md mx-auto space-y-6">
        <div className="flex flex-col items-center py-8 px-4 bg-white/80 backdrop-blur-sm rounded-3xl border border-black/5 shadow-sm">
          ...
          <button onClick={() => navigate('/list')} className="flex items-center gap-2 text-xs font-bold text-blue-700 bg-blue-100/80 px-4 py-2 rounded-full hover:bg-blue-200 transition-colors"><Building2 className="w-3 h-3" /> 現場切替: {currentSiteName}</button>
        </div>
        <div className="space-y-3">
    ...
    <div className="min-h-screen bg-gray-50 p-6 font-sans">
      <div className="max-w-md mx-auto pb-12">
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-black/5 mb-6">
          ...
          <label className="block w-full text-center bg-green-100/50 text-green-700 font-bold py-3 rounded-lg cursor-pointer">画像を選択 <input type="file" accept="image/*" className="hidden" onChange={(e: any) => { if (e.target.files[0]) compressImage(e.target.files[0], (url) => updateCurrentProject('mapImage', url)); }} /></label>
        </div>
...
function PDFExportScreen() {
  ...
  return (
    <div className="min-h-screen bg-gray-200 p-6 font-sans flex flex-col items-center pb-12">
      <div className="w-full max-w-2xl mb-6 flex justify-between items-center"><button onClick={() => navigate('/')} className="text-blue-500 font-bold flex items-center gap-2"><ArrowLeft className="w-5 h-5" /> もどる</button><button onClick={handleExport} className="bg-orange-500 text-white px-6 py-3 rounded-xl font-bold shadow">ダウンロード</button></div>
      <div className="overflow-auto w-full max-w-2xl bg-gray-300 p-4 rounded-xl flex flex-col gap-8">
        <div className="pdf-page bg-white relative shadow-md flex flex-col" style={{ width: '210mm', height: '297mm', padding: '20mm' }}>
          <div className="w-full h-full border-[3px] border-gray-800 p-12 flex flex-col relative"><div className="mt-[30mm] mb-[40mm] text-center"><h1 className="text-4xl font-serif tracking-[0.5em] font-bold mb-4">工事写真報告書</h1><div className="w-[110mm] mx-auto border-b-[2px] border-gray-800"></div><div className="w-[110mm] mx-auto border-b-[1px] border-gray-800 mt-1"></div></div><div className="flex-1 px-4 space-y-14">{[{ label: "工事件名", value: project.projectName }, { label: "工事場所", value: project.projectLocation }, { label: "工期", value: project.constructionPeriod }, { label: "施工業者", value: project.contractorName }, { label: "作成年月日", value: project.creationDate }].map((item, idx) => (<div key={idx} className="flex items-baseline border-b-2 border-gray-800 pb-1"><div className="w-[50mm] flex-shrink-0 flex justify-between text-[20px] font-bold pr-16">{item.label.split('').map((c, i) => <span key={i}>{c}</span>)}</div><div className="flex-1 text-[20px] font-medium whitespace-nowrap overflow-hidden">{item.value || "　"}</div></div>))}</div></div><div className="absolute bottom-[10mm] right-[15mm] text-xs font-serif text-gray-400">- 1 / {totalPages} -</div></div>
        <div className="pdf-page bg-white relative shadow-md flex flex-col" style={{ width: '210mm', height: '297mm', padding: '15mm' }}><div className="w-full h-full border-[3px] border-gray-800 p-6 flex flex-col"><h2 className="text-2xl font-bold mb-4 border-b-2 border-gray-800 pb-2">位置図</h2><div className="h-[45%] border border-gray-400 mb-6 flex items-center justify-center p-2 bg-gray-50">{project.mapImage && <img src={project.mapImage} style={{ maxWidth: '100%', maxHeight: '100%', objectFit: 'contain' }} />}</div><div className="flex-1 flex flex-col"><div className="flex bg-gray-200 border border-gray-800 font-bold text-center text-sm"><div className="w-1/4 border-r border-gray-800 p-2">符号</div><div className="w-1/2 border-r border-gray-800 p-2">部位</div><div className="w-1/4 p-2">写真NO</div></div>{project.mapRows.map((row: any) => (<div key={row.id} className="flex border-b border-l border-r border-gray-800 text-center text-sm"><div className="w-1/4 border-r border-gray-800 p-2">{row.symbol || "-"}</div><div className="w-1/2 border-r border-gray-800 p-2">{row.part}</div><div className="w-1/4 p-2">{row.relatedPhotoNumber || "-"}</div></div>))}</div></div><div className="absolute bottom-[10mm] right-[15mm] text-xs font-serif text-gray-400">- 2 / {totalPages} -</div></div>
